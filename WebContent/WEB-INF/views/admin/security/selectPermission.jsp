...
<html>
	...
	<body>
		...
		$(function(){
			$('#dg').datagrid({
				url:'${ctx}/admin/permission/authorize/?roleId=${roleId}',
				method:'post', // 请求类型
				...
				columns:[[
					...
					{field:'operate',title:'操作',width:100,halign:'center',align:'center',
						formatter:function(val,row){
							if(row.authorize == 0) {
								return '<a href="javascript:authPermission();">授权</a>';
							} else if(row.authorize == 1) {
								return '<a href="javascript:unauthPermission();">撤销授权</a>';
							}
   						}
					}
				]],
			...
			if(pg){
				$(pg).pagination({
					...
					onSelectPage:function(pageNumber,pageSize){
				       	...
				       	var url = encodeURI('${ctx}/admin/permission/authorize/?roleId=${roleId}&rows='+pageSize+'&page='+pageNumber+'&name='+encodeURI(name));
		...
		function searchRole() {
			...
			var max = Math.ceil(total/options.pageSize); // 总页数
           	...
           	var name = $('#search_name').val();
			var url = encodeURI('${ctx}/admin/permission/authorize/?roleId=${roleId}&rows='+currentPageSize+'&page='+currentPageNo+'&name='+encodeURI(name));
	        $.post(url, {}, function(data) {
	            var d = data;
	            $('#dg').datagrid('loadData',d);
	        }, 'json');
		...
		function authPermission(){
		    ...
		    if (row){
				$.ajax({
		    		url: '${ctx}/admin/permission/authPermission/'+row.id,
		    		type: 'post',
		    		data: {id: row.id},
		...
		// 撤销授权权限
		function unauthPermission(){
			var row = $('#dg').datagrid('getSelected');
		    if (row){
				$.ajax({
		    		url: '${ctx}/admin/permission/unauthPermission/'+row.id,
		    		type: 'post',
		    		data: {id: row.id},
		    		contentType : 'application/json',
		    		dataType : "json",
