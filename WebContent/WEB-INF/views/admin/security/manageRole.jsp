...
<html>
	...
	<body>
		<div style="overflow: hidden; padding: 0px;">
			<table id="dg"></table>
			<div id="dicwin" title="选择权限" closed="true"></div>
			<div id="tb" style="padding:1px;height:auto">
				...
				<div style="text-align: right">
					...
					<shiro:hasPermission name="role:delete">
						<a href="#" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="javascript:deleteRole();">删除</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="role:authPermission">
						<a href="#" class="easyui-linkbutton" iconCls="icon-config" plain="true" onclick="javascript:selectPermission();">选择权限</a>
					</shiro:hasPermission>
					<a href="#" class="easyui-linkbutton" iconCls="icon-config" plain="true" onclick="javascript:selectMenu();">选择菜单</a>
				</div>
			</div>
		</div>
		<div id="dlg" class="easyui-dialog" style="width:450px;height:213px;padding:5px" closed="true" buttons="#dlg-buttons">
			<form:form id="fm" modelAttribute="role" method="post">
				<table id="mytable" width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
					<tr>
						<td class="tleft" style="width: 100px; font-size: 12px;">角色名称：</td>
						<td class="tright">
		...
		<div id="dlg-buttons">
			<a id="savebtn" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-save'" onclick="$('#savebtn').linkbutton('disable');javascript:saveRole();">确定</a>
			...
			if(pg){
				$(pg).pagination({
					...
					onSelectPage:function(pageNumber,pageSize){
				       	...
				       	var url = encodeURI('${ctx}/admin/role/?rows='+pageSize+'&page='+pageNumber+'&name='+encodeURI(name));
		...
		function searchRole() {
			...
			var max = Math.ceil(total/options.pageSize); // 总页数
           	...
           	var name = $('#search_name').val();
			var url = encodeURI('${ctx}/admin/role/?rows='+currentPageSize+'&page='+currentPageNo+'&name='+encodeURI(name));
	        $.post(url, {}, function(data) {
	            var d = data;
	            $('#dg').datagrid('loadData',d);
	        }, 'json');
		...
		function deleteRole(){
			var row = $('#dg').datagrid('getSelected');
		    if (row){
		        $.messager.confirm('确认框','您确定要删除角色【'+row.name+'】吗?',function(r){
		            if (r){
						$.ajax({
				    		url: '${ctx}/admin/role/delete/'+row.id,
				    		type: 'post',
				    		data: {id: row.id},
		...
		function selectPermission(){
			var row = $('#dg').datagrid('getSelected');
			if(row){
				selectDictionary('#dicwin','${ctx}/admin/role/selectPermission/'+row.id);
			} else {
		...
		function selectMenu(){
			var row = $('#dg').datagrid('getSelected');
			if(row){
				selectDictionary('#dicwin','${ctx}/admin/role/selectMenu/'+row.id);
			} else {
				$.messager.alert('提示信息','请选择您要操作的数据！');
			}
		}
