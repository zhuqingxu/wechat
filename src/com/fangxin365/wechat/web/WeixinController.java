...
public class WeixinController extends BaseController {
	...
	public String synchrUserlist(HttpServletRequest request, Model model) {
		...
		String synchr_user_list_url = request.getParameter("synchr_user_list_url");
				...
				try {
					...
					if(results.length() > 0) {
						...
						for (int i = 0; i < results.length(); i++) {
							logger.info("序号：" + i);
							weixinUser = weixinUserService.getWeixinUser(results.getString(i));
							if(weixinUser == null) {
	...
	public String synchrUserInfo(HttpServletRequest request, Model model) {
		...
		do{
			...
			logger.info("正在处理第" + currentPageNo + "页，共" + users.getTotalPages() + "页");
			
			for(WeixinUser user : userList) {
				if(StringUtils.isNotBlank(user.getOpenid())) {
					String synchr_url = synchr_user_list_url + access_token + "&openid=" + user.getOpenid();
			...
			if (code == 200) {
				...
				try {
					...
					if("1".equals(subscribe)) {
					    ...
					    Long time = new Long(Long.parseLong(subscribe_time)*1000);  
					    String date = format.format(time);
						
						WeixinUser weixinUser = null;
						if(StringUtils.isNotBlank(openid)) {
							weixinUser = weixinUserService.getWeixinUser(openid);
	...
	public String synchrGroup(HttpServletRequest request, Model model) {
				...
				try {
					...
					if(ja.length() > 0) {
						...
						for (int i = 0; i < ja.length(); i++) {
							logger.info("序号：" + i);
							weixinGroup = weixinGroupService.getWeixinGroup(ja.getJSONObject(i).getString("id"));
							if(weixinGroup == null) {
								weixinGroup = new WeixinGroup();
								weixinGroup.setGroupId(ja.getJSONObject(i).getString("id"));
								weixinGroup.setGroupName(ja.getJSONObject(i).getString("name"));
								weixinGroup.setCount(ja.getJSONObject(i).getString("count"));
